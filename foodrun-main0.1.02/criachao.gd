...
func spawn_segment() -> void:
	var max_safe_gap: float = _get_max_jump_distance() * safety_margin
	var past_safe_zone: bool = next_x >= safe_start_distance

	# Espaço mínimo pra caber 2 pulos + a plataforma no meio
	var min_pit_with_platform: float = (max_safe_gap * 0.6) * 2 + min_obstacle_width_estimate

	if past_safe_zone and randf() < pit_chance and min_pit_with_platform <= (max_safe_gap * 2):
